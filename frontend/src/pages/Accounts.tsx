...
import { Plus, Search, Trash2, FolderTree, AlertCircle, X } from 'lucide-react';
...
export function Accounts() {
  ...
  const { data: accounts = [], isLoading, error } = useQuery<Account[]>({
    ...
    queryFn: async () => {
      ...
      return fetchApi(`/accounts?companyId=${activeCompany.id}`);
    },
  ...
  const createMutation = useMutation({
    mutationFn: async (newAccount: typeof formData) => {
      return fetchApi('/accounts', {
        method: 'POST',
        body: JSON.stringify({ ...newAccount, companyId: activeCompany?.id })
      });
    },
    onSuccess: () => {
  ...
  const deactivateMutation = useMutation({
    mutationFn: async (accountId: string) => {
      return fetchApi(`/accounts/${accountId}?companyId=${activeCompany?.id}`, {
        method: 'DELETE'
      });
    },
    onSuccess: () => {
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`¿Estás seguro de que deseas desactivar la cuenta ${name}?`)) {
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData({ ...formData, accountType: type, normalBalance: balance });
  };
  ...
  const renderAccountGroup = (title: string, type: 'asset' | 'liability' | 'equity' | 'revenue' | 'expense') => {
                ...
                <tr key={acc.id} className="hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-white">
                    <div className="flex items-center gap-2" style={{ paddingLeft: `${(acc.level - 1) * 20}px` }}>
                      ...
                      {acc.name}
                      {acc.is_system === 1 && (
                        <span className="text-[10px] bg-gray-700 text-gray-400 px-1.5 py-0.5 rounded uppercase tracking-wider">Sistema</span>
                      )}
                    </div>
                  </td>
                  <td className={`px-4 py-3 text-right font-mono text-sm ${acc.balance < 0 ? 'text-rose-400' : 'text-gray-300'}`}>
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="animate-pulse bg-gray-800/50 h-32 rounded-lg border border-gray-700/50 w-full" />
          ))}
        </div>
      ) : (
        <div className="pt-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <form id="createAccountForm" className="space-y-5" onSubmit={(e) => {
                e.preventDefault();
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" 
                      placeholder="Código del padre Ej: 1100"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 appearance-none"
                    >
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 resize-none h-20" 
            ...
            <div className="p-6 border-t border-gray-800 bg-gray-900/50 flex justify-end gap-3 shrink-0">
              ...
              <button 
                ...
                disabled={createMutation.isPending}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors shadow-lg shadow-indigo-500/20 flex items-center gap-2 disabled:opacity-50"
              >
